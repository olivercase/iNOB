...
MANUAL_OVERRIDES: dict[str, str] = {
    ...
    "FMA72976": "brain", "FMA72975": "brain",       # occipital lobe L/R
    "FMA72672": "brain", "FMA72671": "brain",       # superior parietal lobule L/R
    "FMA72940": "brain", "FMA61974": "brain",       # stria terminalis L/—
    "FMA62033": "brain",                            # pineal body
    "FMA62327": "brain",                            # tuber cinereum
    # respiratory / digestive / oral organ subparts
    "FMA7396": "organ", "FMA68418": "organ",        # main bronchus L/R
    "FMA11338": "organ",                            # ileocecal junction
    "FMA59816": "organ",                            # lip
    "FMA52781": "organ",                            # external ear
    "FMA14643": "organ", "FMA16549": "organ",       # mesentery, mesoappendix
    "FMA14647": "organ",                            # transverse mesocolon
    "FMA63120": "organ",                            # parenchyma of pancreas
    "FMA15044": "organ", "FMA15042": "organ", "FMA15043": "organ",  # taeniae coli
    # bone
    "FMA7486": "bone",                              # manubrium (of sternum)
    "FMA7488": "bone",                              # xiphoid process
    # muscle (laryngeal)
    "FMA46605": "muscle", "FMA46604": "muscle",     # aryepiglotticus L/R
    # blood vessel (arterial arches)
    "FMA22840": "blood_vessel", "FMA22839": "blood_vessel",  # deep palmar arch L/R
    "FMA43944": "blood_vessel", "FMA43943": "blood_vessel",  # plantar arch L/R
    # fibrous raphes / membranes
    "FMA55620": "ligament_tendon", "FMA55619": "ligament_tendon",  # pterygomandibular raphe
    "FMA55077": "ligament_tendon",                  # pharyngeal raphe
    "FMA55252": "ligament_tendon", "FMA55251": "ligament_tendon",  # conus elasticus L/R
    # cartilage
    "FMA55130": "cartilage",                        # epiglottis (elastic cartilage)
    # skin / integument appendages
    "FMA54237": "skin", "FMA54241": "skin", "FMA54319": "skin",  # eyebrow, hair, pubic hair
}
...
NAME_RULES: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"\b(celiac|coeliac|pulmonary|brachiocephalic|thyrocervical|"
                r"costocervical) trunk\b", re.I), "blood_vessel"),
    # CNS / brain (nervous tissue, distinct from peripheral nerve for FEM).
    (re.compile(r"\b(cerebr|cerebell|cortex|cortical|gyrus|gyri|sulcus|colliculus|"
                ...
                re.I), "brain"),
    (re.compile(r"\b(nerve|nervous|ganglion|ganglia|plexus|\bnucleus\b|nerve tract|"
                r"rootlet|ramus)\b", re.I), "nerve"),
    (re.compile(r"\b(cartilage|meniscus|intervertebral disc|articular disc)\b", re.I), "cartilage"),
    (re.compile(r"\b(ligament|tendon|tendinous|aponeurosis|retinaculum|fascia|"
                r"iliotibial tract|linea alba)\b", re.I), "ligament_tendon"),
    # Named skeletal muscles that the sparse IS-A tree didn't chain to FMA5022.
    (re.compile(r"\b(muscle|muscular|part of (left|right) |levatores|levator|"
                ...
                r"arytenoid|stylohyoid|geniohyoid|sphincter|detrusor)\b", re.I), "muscle"),
    # Muscle-group stems (unanchored: catch plurals like "lumbricals", "rotatores").
    (re.compile(r"(lumbrical|intertransversari|interspinal|rotatores|multifidus|"
                r"semispinalis|splenius|longissimus|iliocostalis|spinalis|"
                r"longus (colli|capitis)|rectus capitis|scalenus|interossei)", re.I), "muscle"),
    (re.compile(r"\b(adipose|fatty|fat pad|\bfat\b)\b", re.I), "fat"),
    (re.compile(r"\b(gland|glandular|pancrea|thyroid|adrenal|parotid|"
                r"submandibular|pituitary|hypophysis)\b", re.I), "gland"),
    ...
    (re.compile(r"\b(bone|vertebra|rib\b|sternum|mandible|maxilla|phalanx|phalange|"
                ...
                r"stapes|calcaneus|talus|navicular|cuboid|cuneiform)\b", re.I), "bone"),
    # Viscera / organ subparts (digestive, respiratory, cardiac, urinary, repro, eye/ear).
    (re.compile(r"\b(colon|ileum|jejunum|duodenum|cecum|caecum|rectum|stomach|"
                ...
                r"peritone)\b", re.I), "organ"),
    (re.compile(r"\b(bronch|lung|trachea|alveol|pleura|larynx|laryngeal|pharyn|"
                r"epiglott|glottis|nasal cavity|paranasal|nostril|nasopharyn)\b", re.I), "organ"),
    (re.compile(r"\b(valve|cusp|leaflet|atrium|atrial|ventricle|ventricular|"
                r"myocard|pericard|endocard|cardiac|chamber of (left|right)|"
                r"interventricular|interatrial|chordae|papillary|auricle)\b", re.I), "organ"),
    (re.compile(r"\b(kidney|renal|ureter|bladder|urethra|urinary|prostat|uterus|"
                ...
                r"fallopian|oviduct|clitoris|labium)\b", re.I), "organ"),
    (re.compile(r"\b(eyeball|cornea|retina|\blens\b|sclera|\biris\b|vitreous|"
                ...
                r"tongue|lingual|tooth|teeth|dental)\b", re.I), "organ"),
    (re.compile(r"\b(heart|trunk of|\bduct\b|sinus|tree|wall of|cavity of|"
    ...
    (re.compile(r"\b(rectus|interosseous|tensor|pectoralis|subclavius|rotator|"
                ...
                re.I), "muscle"),
    (re.compile(r"\b(trapezoid|trapezium|capitate|hamate|lunate|pisiform|scaphoid|"
                r"triquetral|atlas|axis|vomer|ethmoid|nasal concha)\b", re.I), "bone"),
    (re.compile(r"\b(prostate|peritoneum|bronchus|parenchyma|bronchopulmonary)\b",
                re.I), "organ"),
]
...
def classify_by_isa(fma: str, parents: dict[str, set[str]]) -> str | None:
    ...
    while frontier:
        ...
        if hits:
            ...
            for b in ("blood_vessel", "nerve", "muscle", "bone", "cartilage",
                      "ligament_tendon", "skin", "gland", "lymphatic", "fat", "organ"):
                if b in hits:
                    return b
            return hits[0]
...
def run(src: Path, dst: Path, *, copy: bool, counts: bool) -> None:
    ...
    for i, path in enumerate(files):
        ...
        if counts:
            v, f = obj_counts(path)
        rows.append({
            "fma_id": fma, "fj_id": fj, "bp_id": bp, "name": readable,
    ...
    logger.info("DONE — %d meshes; by system: %s", len(rows),
...
def write_readme(dst: Path, by_system: dict[str, int], method: dict[str, int], total: int) -> None:
    ...
    lines += [
        ...
        "",
        "See `MANIFEST.csv` for the per-mesh fma_id / system / method / "
        "vertex+face counts.",
    ]
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--dst", type=Path,
                    default=Path("/Volumes/UCL/Forward_Model_Vagus_Nerve/internal_meshes"))
    ap.add_argument("--no-copy", action="store_true", help="classify + manifest only; don't copy files")
