...
def _patch_all(monkeypatch, calls):
    monkeypatch.setattr(
        cli_mod, "render_surface_topoplots",
        lambda cfg, **kw: calls.setdefault("surface", kw),
    )
    monkeypatch.setattr(
        cli_mod, "render_detectability",
        lambda cfg, **kw: calls.setdefault("detect", kw),
    )
    monkeypatch.setattr(
        cli_mod, "detectability_summary",
...
def test_main_target_surface_only(tmp_path, monkeypatch) -> None:
    ...
    _patch_all(monkeypatch, calls)
    rc = cli_mod.main([
        "--config", str(TINY_CFG), "--project-root", str(tmp_path), "--target", "surface",
    ])
    assert rc == 0
...
def test_main_target_detect_only(tmp_path, monkeypatch) -> None:
    ...
    _patch_all(monkeypatch, calls)
    rc = cli_mod.main([
        "--config", str(TINY_CFG), "--project-root", str(tmp_path), "--target", "detect",
    ])
    assert rc == 0
...
def test_main_forwards_thresholds(tmp_path, monkeypatch) -> None:
    ...
    rc = cli_mod.main([
        "--config", str(TINY_CFG), "--project-root", str(tmp_path),
        "--target", "detect", "--snr-threshold", "5", "--max-trials", "100",
        "--source-idx", "2",
    ])
    assert rc == 0
