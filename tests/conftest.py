...
@pytest.fixture
def tiny_geometry(cube_trimesh: trimesh.Trimesh,
                  sphere_trimesh: trimesh.Trimesh) -> Geometry:
    """Two-compartment geometry: skin (cube) and vagus_left (sphere inside)."""
    return Geometry(compartments={
...
def tiny_sensors() -> SensorArray:
    ...
    labels = tuple(
        f"mag-{i // 3:04d}-{['R', 'T1', 'T2'][i % 3]}" for i in range(12)
    )
    return SensorArray(
        coilpos=coilpos, coilori=coilori, labels=labels,
        chantype=tuple(["megmag"] * 12), chanunit=tuple(["T"] * 12), unit="mm",
    )
