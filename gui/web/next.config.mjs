...
const nextConfig = {
  ...
  async rewrites() {
    return [
      ...
      { source: "/api/levels", destination: `${BACKEND}/api/levels` },
      { source: "/api/sensors", destination: `${BACKEND}/api/sensors` },
      { source: "/api/fieldmap", destination: `${BACKEND}/api/fieldmap` },
      { source: "/api/sources/:path*", destination: `${BACKEND}/api/sources/:path*` },
