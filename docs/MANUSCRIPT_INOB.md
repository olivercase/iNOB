...
**Supplementary Fig. 2 | Fibre-parameter sensitivity.** `[PLACEHOLDER]` Histogram of trials-to-SNR = 3 across Monte Carlo draws from the fibre-parameter space (ΔV ± 10 mV, σ_in ∈ {0.35, 1.0} S/m, diameter distribution ± 1 s.d.). Expected output from `inob.analysis.cap_sensitivity`.
...
|---|---|---|---|
| **Critical** | Conductivity sensitivity sweep results | `inob-sensitivity` (MEG + EEG), save `outputs/sensitivity_{meg,eeg}.json`, generate figure | Fig. 4 |
| **Critical** | Uncertainty bands on detectability | Monte Carlo in `inob.analysis.cap_sensitivity` over ΔV, σ_in, diameter distribution; add shaded bands | Fig. 5 |
...
| **Important** | Fibre-parameter Monte Carlo | `inob.analysis.cap_sensitivity`; Supplementary Fig. 2 | Supplementary Fig. 2 |
| **Important** | Contact impedance sensitivity | `inob.analysis.eeg_impedance_sensitivity`; Supplementary Fig. 3 | Supplementary Fig. 3 |
