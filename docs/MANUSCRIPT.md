...
Two physiologically-grounded scenarios are implemented (`inob.physiology.scenarios`):
...
The simulator (`inob.physiology.simulate`) produces a per-channel time series at every sensor by superposing biphasic-Gaussian compound action potentials at the cervical-source hot-spot, with fs = 30 kHz default and AP width σ = 0.5 ms.
