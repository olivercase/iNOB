...
from vagus_fm.io.hdf5 import load_geometry, load_sensors
...
def render_cross_modality(
    ...
    dpi: int = 300, noise_uV: float | None = None, noise_seed: int = 0,
) -> Path:
    ...
    snr_db = 20.0 * np.log10(
        max(np.linalg.norm(V_eeg_obs_clean), 1e-30) / max(noise_uV * np.sqrt(len(V_eeg_obs_clean)), 1e-30)
    )
    ...
    gs = GridSpec(2, 2, figure=fig, left=0.05, right=0.97, top=0.93, bottom=0.06,
                  hspace=0.30, wspace=0.22)

    # ── panel a: amplitude scatter ─────────────────────────────────────────
    ax_a = fig.add_subplot(gs[0, 0])
    z = meg_lf.source_pos[:, 2]
    sc = ax_a.scatter(a_eeg, a_meg, c=z, cmap="magma",
                      s=22, edgecolor=NATURE_PALETTE["axis"], linewidths=0.2)
    ax_a.scatter([a_eeg[source_idx]], [a_meg[source_idx]],
    ...
    _draw_eeg_2d_topoplot(ax_b, electrodes, V_eeg_obs, cfg,
                          vmin=vmin_b, vmax=vmax_b)
    ax_b.set_title(f"Observed EEG  ·  source z = {src[2]:.0f} mm  "
                    f"(longitudinal moment)")
    ...
    sc_c = ax_c.scatter(
        ...
        c=pred_vals, cmap=divergent_cmap(), vmin=vmin_c, vmax=vmax_c,
        s=44, edgecolor=NATURE_PALETTE["axis"], linewidths=0.25,
        depthshade=False,
    )
    ax_c.scatter([src[0]], [src[1]], [src[2]],
