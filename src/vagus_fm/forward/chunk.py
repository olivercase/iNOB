...
The companion :mod:`vagus_fm.forward.reduce` stitches all chunks back into a
single leadfield NPZ identical in schema to the local
:mod:`vagus_fm.forward.solve` output (``source_pos`` included).
"""
...
def main(argv: list[str] | None = None) -> int:
    """``python -m vagus_fm.forward.chunk`` entry point (used by submit_array.sh)."""
    import argparse
