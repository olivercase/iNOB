...
The companion of :mod:`vagus_fm.forward.solve` (MEG). Same FEM, same source
...
produced by :func:`vagus_fm.forward.solve.run_forward` with the magnetic
...
# :mod:`vagus_fm.analysis.sphere_calibration`. See ``outputs/calibration/
...
def run_eeg_forward(cfg: Config) -> Path:
    ...
    logger.info("%d sources along %s (%.1f mm spacing)",
                len(src_pos_mm), cfg.forward.source_tissue,
                cfg.forward.source_spacing_mm)
    ...
    # The factor is set empirically by ``vagus_fm.analysis.sphere_calibration``
