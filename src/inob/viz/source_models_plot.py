...
def _bar_pair(ax, values_meg, values_eeg, labels, *, log: bool = False):
    ...
    ax.bar(x + w / 2, values_eeg, w, color=EEG_COLOUR, label="electrodes (best bipolar)")
    ax.set_xticks(x)
...
def render_source_models(
    ...
    *,
    ...
    gs = GridSpec(1, 3, figure=fig, left=0.06, right=0.985, top=0.82, bottom=0.20, wspace=0.28)

    # ── a: single-trial SNR ────────────────────────────────────────────────
    ax_a = fig.add_subplot(gs[0, 0])
    _bar_pair(ax_a, [r.meg_snr for r in cmp.rows], [r.eeg_snr for r in cmp.rows], labels, log=True)
    ...
    ax_a.text(
        0.98,
        SNR_TARGET * 1.2,
        f"SNR = {SNR_TARGET:g} (Rose criterion)",
    ...
    _bar_pair(
        ax_b, [r.meg_trials for r in cmp.rows], [r.eeg_trials for r in cmp.rows], labels, log=True
    )
    ...
    if budget is not None:
        ...
        ax_b.text(
            0.02,
            budget[1],
            f"  clinical budget {budget[0]}–{budget[1]}",
    ...
    ratio_values = [r.modality_gap for r in cmp.rows] + [
        cmp.meg_propagation_factor,
        cmp.eeg_propagation_factor,
