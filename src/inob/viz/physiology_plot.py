...
def _row_for_modality(
    ...
    *,
    ...
    modality: str,
    label_prefix: str,
    panel_letters: tuple[str, str, str],
):
    ...
    ax_a.plot(
        sim.t_s,
        trace,
        color=(NATURE_PALETTE["blue"] if modality == "meg" else NATURE_PALETTE["red"]),
        lw=0.8,
    )
    ax_a.axhline(
        sigma, color=NATURE_PALETTE["axis"], lw=0.5, linestyle="--", label=f"σ = {sigma:.1f} {unit}"
    )
    ...
    ax_a.set_title(
        f"{label_prefix}  ·  best channel #{best_c}  ·  single-trial peak-SNR = {snr_single:.2f}"
    )
...
def render_physiology(
    ...
    *,
    ...
    for row, sc in enumerate(scenarios):
        ...
        meg_best = int(np.argmax(np.sqrt(np.mean(meg_h**2, axis=1)) * meg_radial))
        ...
        ax1.plot(
            eeg_sim.t_s,
            eeg_h[eeg_best] / sigma_eeg,
            color=NATURE_PALETTE["red"],
            lw=0.8,
            label=f"EEG / σ_EEG  (peak {np.abs(eeg_h[eeg_best]).max() / sigma_eeg:.2e})",
        )
        ax1.axhline(3, color=NATURE_PALETTE["axis"], lw=0.6, linestyle="--", label="SNR=3")
        ...
        ax2.axhline(3, color=NATURE_PALETTE["axis"], lw=0.6, linestyle="--", label="SNR = 3")
        ax2.set_xscale("log")
        ax2.set_yscale("log")
        ...
        mask = fft_freq <= 10.0
        ax3.plot(fft_freq[mask], fft_mag[mask], color=NATURE_PALETTE["blue"], lw=0.8)
        ax3.axvline(
            sc.rate_hz,
            color=NATURE_PALETTE["red"],
            lw=0.8,
            linestyle="--",
            label=f"f₀ = {sc.rate_hz:.2f} Hz",
        )
